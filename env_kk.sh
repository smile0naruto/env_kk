...
{
	if [ "$1" = "help" ]; then 
		echo "this is cmd function for kk-drivers debug. "
	fi
	...
	kk_module=$2
	kk_mmb=$3
	PROJECT=$(echo $TARGET_PRODUCT | cut -b 6- )
	AOSP_KERNEL_OUT=$ANDROID_PRODUCT_OUT/obj/KERNEL_OBJ
	AOSP_LK_OUT=$ANDROID_PRODUCT_OUT/obj/BOOTLOADER_OBJ/build-$PROJECT
	AOSP_PL_OUT=$ANDROID_PRODUCT_OUT/obj/PRELOADER_OBJ

	if [ "$kk_option" = "cp" ]; then
		case $kk_module in
	     	dws)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is copy dws file to lk & kernel from pl!!!!"
				show_cmd cp vendor/mediatek/proprietary/bootable/bootloader/preloader/custom/$PROJECT/dct/dct/codegen.dws kernel-3.18/drivers/misc/mediatek/mach/mt6735/$PROJECT/dct/dct/codegen.dws 
				show_cmd cp vendor/mediatek/proprietary/bootable/bootloader/preloader/custom/$PROJECT/dct/dct/codegen.dws vendor/mediatek/proprietary/bootable/bootloader/lk/target/$PROJECT/dct/dct/codegen.dws 
				;;
			*) show_cmd echo "input with unknow parameters!!!";;
		esac
	elif [ "$kk_option" = "rm" ]; then 
		case $kk_module in
			power)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm power in out-kernel!!!!"
				show_cmd rm $AOSP_KERNEL_OUT/drivers/misc/mediatek/power/mt6735/ -rf
				show_cmd rm $AOSP_KERNEL_OUT/drivers/power/mediatek/ -rf
				show_cmd echo "rm power success!"
				;;
							alsp)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm alsp in out-kernel!!!!"
				show_cmd rm $AOSP_KERNEL_OUT/drivers/misc/mediatek/alsps/ -rf
				show_cmd echo "rm alsp success!"
				;;
			tp)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm tp in out-kernel!!!!"
				show_cmd rm $AOSP_KERNEL_OUT/drivers/input/touchscreen/mediatek/ -rf
				show_cmd echo "rm tp success!"
				;;
			camera)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm camera in out-kernel!!!!"
				show_cmd rm $AOSP_KERNEL_OUT/drivers/misc/mediatek/imgsensor/ -rf
				show_cmd echo "rm camera success!"
				;;
			lk)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm lk in out-lk!!!!"
				show_cmd rm $AOSP_LK_OUT/* -rf
				show_cmd echo "rm lk success!"
				;;
			kernel)
				show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm kernel in out-kernel!!!!"
				show_cmd rm $AOSP_KERNEL_OUT/* -rf
				show_cmd echo "rm kernel success!"
				;;
			pl)
			    show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm pl in out-pl!!!!"
				show_cmd rm $AOSP_PL_OUT/* -rf
				show_cmd echo "rm pl success!"
				;;
			klp)
			    show_cmd cd ${ANDROID_BUILD_TOP}
				show_cmd echo "this option is rm klp in out-pl!!!!"
				show_cmd rm $AOSP_PL_OUT/* -rf
				show_cmd rm $AOSP_KERNEL_OUT/* -rf
				show_cmd rm $AOSP_LK_OUT/* -rf
				show_cmd echo "rm klp success!"
				;;				
			*) show_cmd echo "input with unknow parameters!!!";;
        esac
	elif [ "$kk_option" = "mm" ]; then 
		case $kk_module in
			kernel)
				show_cmd cd ${ANDROID_BUILD_TOP}
				if [ "$kk_mmb" = "b" ]; then 
				show_cmd mmm -B kernel-3.18:kernel -j64 && show_cmd make bootimage-nodeps 
				else
				show_cmd mmm kernel-3.18:kernel -j64 && show_cmd make bootimage-nodeps
				fi
				;;
			lk)
				show_cmd cd ${ANDROID_BUILD_TOP}
				if [ "$kk_mmb" = "b" ]; then 
				show_cmd mmm -B vendor/mediatek/proprietary/bootable/bootloader/lk:lk -j64
				else
				show_cmd mmm vendor/mediatek/proprietary/bootable/bootloader/lk:lk -j64
				fi
				;;
			pl)
				show_cmd cd ${ANDROID_BUILD_TOP}
				if [ "$kk_mmb" = "b" ]; then 
				show_cmd mmm -B vendor/mediatek/proprietary/bootable/bootloader/preloader:pl -j64
				else
				show_cmd mmm vendor/mediatek/proprietary/bootable/bootloader/preloader:pl -j64
				fi
				;;
			klp)
				show_cmd cd ${ANDROID_BUILD_TOP}
				if [ "$kk_mmb" = "b" ]; then
				show_cmd mmm -B kernel-3.18:kernel -j64 && show_cmd make bootimage-nodeps && mmm -B vendor/mediatek/proprietary/bootable/bootloader/preloader:pl -j64 && mmm -B vendor/mediatek/proprietary/bootable/bootloader/preloader:pl -j64
				else
				show_cmd mmm kernel-3.18:kernel -j64 && show_cmd make bootimage-nodeps && mmm vendor/mediatek/proprietary/bootable/bootloader/preloader:pl -j64 && mmm vendor/mediatek/proprietary/bootable/bootloader/preloader:pl -j64
				fi
				;;
			*) show_cmd echo "input with unknow parameters!!!";;
        esac
	elif [ "$kk_option" = "type" ]; then
